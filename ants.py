    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if isinstance(insect, QueenAnt) and insect.true_queen:
                return
            if self.ant.container and self.ant.ant != None:
...
class AntColony:
    ...
    def configure(self, hive, create_places):
        ...
        # print (QueenAnt.real_queen.place)
        # self.queen = QueenPlace(Place('ColonyQueen'), Place('AntQueen'))
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        while next_place != None:
            ant = next_place.ant
            if ant != None and ant != self:
                if not ant in QueenAnt.ants_behind:
                    ant.damage *= 2
                    QueenAnt.ants_behind.append(ant)
                    if ant.container:
                        if ant.ant != None:
                            if ant.ant != self:
                                if not ant.ant in QueenAnt.ants_behind:
                                    ant.ant.damage *= 2
                                    QueenAnt.ants_behind.append(ant)
